...
| 角色                             | 说明                             |
| -------------------------------- | -------------------------------- |
| 抽象接口（Abstraction）          | 定义高层业务逻辑，持有实现层引用 |
| 细化抽象（Refined Abstraction）  | 扩展抽象接口的具体业务变体       |
| 实现接口（Implementor）          | 定义实现维度的底层操作           |
| 具体实现（Concrete Implementor） | 实现接口的具体底层逻辑           |
...
| 维度         | 桥接模式                                 | 适配器模式                               |
| ------------ | ---------------------------------------- | ---------------------------------------- |
| 设计时机     | 设计阶段主动使用，预留两个维度的扩展空间 | 开发后期补救，解决已有类的接口不兼容问题 |
| 目的         | 解耦抽象与实现，让两个维度独立演化       | 接口转换，让不兼容的类协同工作           |
| 是否改变接口 | 不改变，双方接口在设计时就对齐           | 改变，适配器将一个接口翻译为另一个       |
| 新增类原因   | 系统天然存在两个独立变化维度             | 复用已有类但接口不兼容                   |
| 组合关系     | 抽象层主动持有实现层引用（自上而下）     | 适配器包装被适配者（平级包装）           |
| 典型场景     | 形状×颜色、武器×附魔、数据库抽象×驱动    | 旧SDK适配新接口、第三方库集成            |

> **本教程示例**：`IWeapon` 持有 `IEnchantment` 引用——武器维度和附魔维度在设计阶段就解耦，这是桥接；若后期需要接入一个接口不兼容的已有武器类，则需要适配器。
